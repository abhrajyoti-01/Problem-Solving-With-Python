...
cleaned_frame["Result"] = cleaned_frame[SUBJECT_COLUMNS].ge(40).all(axis=1).map({True: "Pass", False: "Fail"})

print("\nAverage score per subject:")
print(cleaned_frame[SUBJECT_COLUMNS].mean())

print("\nTop 5 students by total score:")
print(cleaned_frame.nlargest(5, "Total")[["StudentID", "Name", "Total"]])

print("\nGender-wise average marks:")
print(cleaned_frame.groupby("Gender")[SUBJECT_COLUMNS].mean())
...
subject_scores = cleaned_frame.melt(id_vars=["StudentID", "Name"], value_vars=SUBJECT_COLUMNS, var_name="Subject", value_name="Score")
...
axes[1, 0].set(title="Attendance Distribution", xlabel="Attendance Percentage", ylabel="Frequency")

axes[1, 1].plot(SUBJECT_COLUMNS, [trend_student[column] for column in SUBJECT_COLUMNS], marker="o", color="tab:green")
axes[1, 1].set_title(f"Academic Trend for {trend_student['Name']}")
