...
cleaned_frame["Active"] = cleaned_frame["Active"].fillna(cleaned_frame["Confirmed"] - cleaned_frame["Recovered"] - cleaned_frame["Deaths"])
...
latest_state_totals = cleaned_frame.sort_values("Date").groupby("State", as_index=False).tail(1).sort_values("Confirmed", ascending=False)
...
print(f"\nHighest impacted state: {highest_impacted_state['State']} ({int(highest_impacted_state['Confirmed'])} confirmed cases)")
...
for state in top_states:
    ...
    axes[0, 0].plot(state_data["Date"], state_data["Confirmed"], marker="o", label=state)
axes[0, 0].set(title="Confirmed Cases Trend for Top 3 States", xlabel="Date", ylabel="Confirmed Cases")
axes[0, 0].legend()
...
axes[1, 0].pie(death_chart["Deaths"], labels=death_chart["State"], autopct="%1.1f%%", startangle=90)
axes[1, 0].set_title("Deaths Proportion Across Top 5 States")

axes[1, 1].scatter(scatter_frame["DailyNewCases"], scatter_frame["DailyRecovered"], color="tab:red")
axes[1, 1].set(title="Daily New Cases vs Daily Recovered", xlabel="Daily New Cases", ylabel="Daily Recovered")
